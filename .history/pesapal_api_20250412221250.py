...
SUBMIT_ORDER_URL = 'https://pay.pesapal.com/v3/api/Transactions/SubmitOrderRequest'  # Production URL
...
def submit_order():
    ...
    order_id = request.json.get('order_id')  # Unique order ID
    currency = request.json.get('currency')  # Currency (e.g., 'KES')
    amount = request.json.get('amount')  # Payment amount
    description = request.json.get('description')  # Description of the order
    callback_url = request.json.get('callback_url')  # Callback URL after payment
    notification_id = request.json.get('notification_id')  # Notification ID (IPN)
    branch = request.json.get('branch')  # Branch name (optional)
    billing_address = request.json.get('billing_address')  # Customer billing address

    if not all([order_id, currency, amount, description, callback_url, notification_id, billing_address]):
    ...
    # Payload for submitting the order request
    payload = {
        'id': order_id,
        'currency': currency,
        'amount': amount,
    ...
    # Send the POST request to submit the order
