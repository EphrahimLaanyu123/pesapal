...
@app.route('/get_ipn_list', methods=['GET'])
def get_ipn_list():
    # Get token from /get_token route first
    token = request.headers.get('Authorization')  # Token should be passed in the 'Authorization' header
    ...
    # Send GET request to fetch the registered IPN URLs
    ...
    if response.status_code == 200:
        ...
        return jsonify({
            'error': 'Failed to fetch IPN list',
            'status_code': response.status_code,
            'message': response.text
        }), 400
