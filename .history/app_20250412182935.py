...
def initiate_payment():
    ...
    if not product_id or not customer_email or not phone_number:
    ...
    # Step 1: Authenticate with Pesapal to get the token
    ...
    try:
        ...
        if not access_token:
            return jsonify({'error': 'Failed to retrieve access token from Pesapal'}), 500

    except requests.exceptions.HTTPError as http_err:
        return jsonify({'error': 'HTTP error during authentication', 'details': str(http_err)}), 500
    except requests.exceptions.RequestException as req_err:
        return jsonify({'error': 'Error with request during authentication', 'details': str(req_err)}), 500
    except Exception as e:
        return jsonify({'error': 'Unexpected error during authentication', 'details': str(e)}), 500
    
    # Step 2: Register IPN (Instant Payment Notification) with Pesapal
    ...
    try:
        ...
        if not notification_id:
            return jsonify({'error': 'Failed to register IPN with Pesapal'}), 500

    except requests.exceptions.HTTPError as http_err:
        return jsonify({'error': 'HTTP error during IPN registration', 'details': str(http_err)}), 500
    except requests.exceptions.RequestException as req_err:
        return jsonify({'error': 'Error with request during IPN registration', 'details': str(req_err)}), 500
    except Exception as e:
        return jsonify({'error': 'Unexpected error during IPN registration', 'details': str(e)}), 500
    ...
    try:
        ...
        orders[order_id] = {
            "pesapal_tracking_id": order_response_data.get('order_tracking_id'),
        ...
        return jsonify({
            "redirect_url": order_response_data.get('redirect_url'),
            "order_id": order_id
        })

    except requests.exceptions.HTTPError as http_err:
        return jsonify({'error': 'HTTP error during order submission', 'details': str(http_err)}), 500
    except requests.exceptions.RequestException as req_err:
        return jsonify({'error': 'Error with request during order submission', 'details': str(req_err)}), 500
    except Exception as e:
        return jsonify({'error': 'Unexpected error during order submission', 'details': str(e)}), 500
